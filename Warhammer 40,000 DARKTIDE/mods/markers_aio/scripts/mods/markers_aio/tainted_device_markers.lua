...
mod.get_max_distance = function()
    local max_distance = mod:get("tainted_max_distance")

    -- foundya Compatibility
    if FoundYa ~= nil then
        max_distance = FoundYa:get("max_distance_book") or mod:get("tainted_max_distance") or 30
    end

    if max_distance == nil then
        max_distance = mod:get("tainted_max_distance") or 30
    end
