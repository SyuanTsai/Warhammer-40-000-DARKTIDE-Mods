    ...
    mod:hook_require(
        "scripts/extension_systems/unit_templates", function(instance)
            if is_mod_loading then
                ...
                mod:hook_safe(
                    instance.medical_crate_deployable, "local_unit_spawned", function(unit, template_context, game_object_data, side_id, deployable, placed_on_unit, owner_unit_or_nil)
                        mod.add_medkit_marker_and_proximity(nil, unit)
                    end
...
mod.update_ammo_med_markers = function(self, marker)
    ...
    if marker and self then
        ...
        if pickup_type and pickup_type == "small_clip" or pickup_type and pickup_type == "large_clip" or pickup_type and pickup_type == "small_grenade" or pickup_type and pickup_type == "ammo_cache_pocketable" or pickup_type and pickup_type == "medical_crate_pocketable" or pickup_type and pickup_type == "medical_crate_deployable" or pickup_type and pickup_type == "ammo_cache_deployable" or marker.type == MarkerTemplate.name or marker.data and marker.data.type == "small_clip" or marker.data and
            marker.data.type == "large_clip" or marker.data and marker.data.type == "small_grenade" or marker.data and marker.data.type == "ammo_cache_pocketable" or marker.data and marker.data.type == "medical_crate_pocketable" or marker.data and marker.data.type == "medical_crate_deployable" or marker.data and marker.data.type == "ammo_cache_deployable" or marker.data and marker.data._active_interaction_type == "health_station" then
            ...
            marker.template.fade_settings.distance_min = marker.template.max_distance - (marker.template.evolve_distance or 0) * 8
            ...
            elseif pickup_type == "medical_crate_deployable" or marker.type == MarkerTemplate.name or marker.data and marker.data.type == "medical_crate_deployable" then
                if marker.widget.style.ring then
                    marker.widget.style.ring.color = mod.lookup_colour(mod:get("med_crate_border_colour"))
                end
                marker.widget.content.icon = "content/ui/materials/hud/interactions/icons/pocketable_medkit"
...
mod.get_proximityheal_medcrate_charges = function(unit)
    ...
    if game_session and game_object_id then
        if GameSession.has_game_object_field(game_session, game_object_id, "heal_reserve") and GameSession.has_game_object_field(game_session, game_object_id, "amount_of_damage_healed") then
